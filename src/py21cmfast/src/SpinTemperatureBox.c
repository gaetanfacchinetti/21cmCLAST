        ...
        if(user_params->USE_INTERPOLATION_TABLES) {
          if(user_params->FAST_FCOLL_TABLES){
            ...
            if(flag_options->M_MIN_in_Mass || flag_options->USE_MASS_DEPENDENT_ZETA) {
                if (flag_options->USE_MINI_HALOS){
                    initialiseSigmaMInterpTable(global_params.M_MIN_INTEGRAL/50.,1.0e+21);
                }
                else{
                    ...
                    num_threads(user_params->N_THREADS)
                {
                    ...
                    for (box_ct=0; box_ct<HII_TOT_NUM_PIXELS; box_ct++){
                        ...
                        if(R_ct==0) {
                            ...
                            rho_B_t = pow(cosmo_params->PMF_SIGMA_A * pow(pow(10.0, cosmo_params->LOG10_PMF_SB)/cosmo_params->PMF_SIGMA_A, 2.0/(5.0+cosmo_params->PMF_NB)), 2) / (2.0 * MU_0) / (C*C); // in g / cm^3
                            ...
                            chiB += -(previous_spin_temp->dpmf_ad_dt_ave + previous_spin_temp->dpmf_turb_dt_ave)/(2*chiB) * dt_dzp * dzp;
